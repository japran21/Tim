...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $asal = $asal_daerea ? "'" . mysqli_real_escape_string($koneksi, $asal_daerah) . "'" : "NULL";
        $query = "INSERT INTO produk (id_umkm, nama_produk, harga, kategori_produk, asal_daerah) 
                  VALUES ($id_umkm, '$nama_produk', $harga, '$kategori_produk', $asal)";
        
        if (mysqli_query($koneksi, $query)) {
            $id_produk = mysqli_insert_id($koneksi);
            $_SESSION['message'] = 'Produk "' . htmlspecialchars($nama_produk) . '" berhasil ditambahkan!';
...
<head>
  ...
  <title>Tambah Produk - UMKM Ciwaruga</title>
...
<body>
  ...
  <div class="form-container">
    <div class="form-card">
      ...
      <p class="form-subtitle">Silakan masukkan data produk makanan/minuman UMKM</p>
      ...
      <form method="POST" action="">
        ...
        <div class="form-group">
          ...
          <input type="text" id="nama_produk" name="nama_produk" placeholder="Contoh: Cimol Bojot, Es Teh Jumbo"
            required>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" id="harga" name="harga" placeholder="10000" step="500" required>
          </div>
        ...
        <div class="form-group">
          <label for="asal_daerah">Asal Daerah (Optional)</label>
          <input type="text" id="asal_daerah" name="asal_daerah" placeholder="Contoh: Bandung, Jawa Barat">
        </div>

        <div class="form-actions">
          <button type="submit" class="btn-submit">Simpan & Lanjut ke Rasa</button>
          <a href="produk.php" class="btn-cancel">Batal</a>
        </div>
      </form>

      <div class="info-card">
        💡 <strong>Tips:</strong> Setelah menambah produk, Anda akan diarahkan ke halaman pengaturan rasa untuk produk
        ini.
      </div>
    </div>
  </div>
